...
worker.registerMaterial = function( description ) {
	_materials[ description.id ] = description;
};

worker.setGravity = function( description ) {
	world.setGravity(new Ammo.btVector3( description.x, description.y, description.z ));
};

worker.addObject = function( description ) {
	var i,
	...
	worker.onmessage({'data':{ cmd: 'objectReady', params: body.id }});
};

worker.removeObject = function( details ) {
	world.removeRigidBody( _objects[details.id] );
	delete _objects[details.id];
	_num_objects--;
};

worker.updateTransform = function( details ) {
...
worker.updateMass = function( details ) {
...
worker.setAngularVelocity = function ( details ) {
...
worker.setLinearVelocity = function ( details ) {
...
worker.setAngularFactor = function ( details ) {
	_objects[details.id].setAngularFactor(
		new Ammo.btVector3( details.x, details.y, details.z )
	);
};

worker.setLinearFactor = function ( details ) {
	_objects[details.id].setLinearFactor(
		new Ammo.btVector3( details.x, details.y, details.z )
	);
};

worker.setDamping = function ( details ) {
	_objects[details.id].setDamping( details.linear, details.angular );
};

worker.setCcdMotionThreshold = function ( details ) {
	_objects[details.id].setCcdMotionThreshold( details.threshold );
};

worker.setCcdSweptSphereRadius = function ( details ) {
	_objects[details.id].setCcdSweptSphereRadius( details.radius );
};

worker.simulate = function( params ) {
	if ( world ) {
		params = params || {};
...
reportWorld = function() {
	...
	worker.onmessage({'data': worldreport} );
};
...
reportCollisions = function() {
	...
	worker.onmessage({'data':collisionreport} );
};
...
worker.postMessage = function( event ) {
	...
	// if ( event.data.cmd && worker[event.data.cmd] ) {
		...
		// worker[event.data.cmd]( event.data.params );
	// }
